...
Template.addGroup.helpers({
  customerShow : function(){ 
  	return Customer.find({_id: { $nin: [Meteor.userId()] }}) },
  customerinSHow : function(){
    return Participants.find({_id: { $nin: [Meteor.userId()] }});
  },
...
console.log("dfsdfsd");
  ...
  var a = Customer.findOne({_id:"tBbDxsHyeixQefy6E"});
  console.log(a);
  Participants.insert(Meteor.user());
  console.log(Meteor.user());
  sAlert.success(Meteor.user().username+"успішно доданий по дефолту як кріейтор");
  
};*/

Template.addGroup.events({
  "click .addcustomer": function () {
    var a = Customer.findOne({_id:this._id});
    //delete a['_id'];
  ...
  "click .removecustomer": function () {
    //var a = Customer.findOne({_id:this._id});
    //delete a['_id'];
    Participants.remove({_id:this._id});
  },
  ...
  sAlert.success(Meteor.user().username+"успішно доданий по дефолту як кріейтор");
    ...
    Meteor.call("createGroup", groupName, Participants.find({}).fetch(), template.dataUrl.get(), function(err){
      if (err){
        console.log("error");
      } else {
        Router.go("/");
        sAlert.success("thx for creating a group!");
      }
    });
  },
...
Template.addGroup.created=function(){
